...
export function isRetryableSentryStatus(status) {
...
export function parseRateLimitResetMs(resetHeader) {
    if (!resetHeader) {
        return null;
    }
...
export function computeRetryDelayMs(attemptIndex, response) {
    ...
    const resetDelay = parseRateLimitResetMs(response.headers.get("X-Sentry-Rate-Limit-Reset"));
    if (resetDelay !== null && resetDelay > 0) {
        return Math.min(resetDelay, DEFAULT_MAX_RETRY_DELAY_MS);
    }
    const exponentialDelay = DEFAULT_INITIAL_RETRY_DELAY_MS * 2 ** attemptIndex;
...
export class SentryApiClient {
    ...
    async makeRequest(endpoint, options = {}) {
        ...
        for (let attemptIndex = 0; attemptIndex < DEFAULT_MAX_RETRY_ATTEMPTS; attemptIndex += 1) {
            ...
            console.error(`Sentry API ${response.status} on ${endpoint}, retry ${attemptIndex + 1}/${DEFAULT_MAX_RETRY_ATTEMPTS - 1} in ${retryDelayMs}ms`);
            await response.text();
