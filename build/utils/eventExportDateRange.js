...
export function normalizeUtcDateString(inputValue) {
...
export function normalizeUtcUntilDateString(inputValue) {
    const plainDatePattern = /^\d{4}-\d{2}-\d{2}$/;
...
export function defaultUtcUntilNow() {
    return new Date().toISOString();
}
export function resolveEffectiveUntilUtc(until) {
    return until ? normalizeUtcUntilDateString(until) : defaultUtcUntilNow();
}
export function buildIssueEventsListQuery(sinceUtc, untilUtc) {
    return {
        start: sinceUtc,
