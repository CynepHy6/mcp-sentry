...
async function resolveIssueContext(apiClient, issueIdOrUrl, organizationSlug, since) {
    const issueReference = extractIssueReference(issueIdOrUrl, organizationSlug);
    const issueDetails = await apiClient.getIssue(issueReference.organizationSlug, issueReference.issueId);
    return {
        ...issueReference,
...
function createExportFilePath(issueContext, exportKind, outputDirectory) {
    ...
    return path.join(baseDirectory, `${exportKind}-issue-${issueLabel}-since-${sinceLabel}-${timestampLabel}.jsonl`);
}
...
export async function exportIssueEventsToFile(apiClient, inputValues) {
    const issueContext = await resolveIssueContext(apiClient, inputValues.issueIdOrUrl, inputValues.organizationSlug, inputValues.since);
